...
const { dbConfig, dbOffConfig,dbHomeConfig } = require("./dbConfig.js");
...
async function connectToHomeDatabase() {
  return new Promise((resolve, reject) => {
    const connectionOff = mysql.createConnection(dbHomeConfig);

    connectionOff.connect((err) => {
      if (err) {
        console.error("Erro ao conectar ao servidor home:", err);
        reject(err);
      } else {
        console.log("Conectado ao banco de dados MySQL home");
        resolve(connectionOff);
      }
    });
  });
...
module.exports = { connectToDatabase, connectToOfflineDatabase, query,connectToHomeDatabase };
